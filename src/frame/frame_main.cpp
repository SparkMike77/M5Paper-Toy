...
void key_calculator_cb(epdgui_args_vector_t &args) {
    Frame_Base *frame = EPDGUI_GetFrame("Frame_Calculator");
    if (frame == NULL) {
        frame = new Frame_Calculator();
        EPDGUI_AddFrame("Frame_Calculator", frame);
    }
    EPDGUI_PushFrame(frame);
    *((int*)(args[0])) = 0;
}
...
Frame_Main::Frame_Main(void): Frame_Base(false) {
    ...
    *(_key[kKeyFactoryTest]->CanvasPressed()) = *(_key[kKeyFactoryTest]->CanvasNormal());
    ...
    _key[kKeyCalculator]->setLabel("Calc");
    _key[kKeyCalculator]->AddArgs(EPDGUI_Button::EVENT_RELEASED, 0, (void*)(&_is_run));
    _key[kKeyCalculator]->Bind(EPDGUI_Button::EVENT_RELEASED, key_calculator_cb);
...
void Frame_Main::AppName(m5epd_update_mode_t mode) {
    ...
    _names->drawString("Calc", 20 + 46 + 3 * 136, 16);
